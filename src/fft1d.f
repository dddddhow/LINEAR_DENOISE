...
C
C   USAGE :  CALL FFTCC(A,N)
C
C   ARGUMENTS : A    COMPLEX VECTOR OF LENGTH N,
C                    ON INPUT A CONTAINS THE
C                    COMPLEX VALUED SEQUENCE TO BE
C                    TRANSFORMED.
C                    ON OUTPUT A IS REPLACED BY THE
C                    FOURIER TRANSFORM.
C               N    INPUT NUMBER OF DATA TO BY
C                    TRANSFORMED. N MAY BE ANY POSITIVE
C                    INTEGER.
C
C  NOTATION :   INFORMATION ON SPECIAL NOTATION AND
C               CONVENTIONS IS AVAILABLE IN THE MANUAL
C               INTRODUCTION OR THROUGH ROUTIME HELP
C
C  REMARKS : 1. FFTCC COMPUTES THE FOURIER TRANSFORM,X,
C               ACCORDING TO THE FOLLOWING FORMULA;
C
C              X(K+1)= SUM FORM J=0 TO N-1 OF
C                      A(J+1)*CEXP((0.0,(2.0*PI*J*K)/N))
C             FOR K=0,1,...,N-1 AND PI=3.1415926...
C
C             NOTE THAT X OVERWRITES A ON OUTPUT.
C
C           2. FFTCC CAN BE USED TO COMPUTE
C
C             X(K+1)= (1/N)*SUM FORM J = 0 TO N-1 OF
C                     A(J+1)*CEXP((0.0,(-2.0*PI*J*K)/N))
C             FOR K=0,1,...,N-1 AND PI=3.1415926...
C
...
*==========================================================================
      ...
      SUBROUTINE ONE_TRACE_FFT(TRACE, LT, LTCOM,
     +     NW, NW1, NW2, NW3, NW4, DATAW)

      REAL TRACE(LT)
      COMPLEX CTRACE(10000)
      COMPLEX DATAW(NW)
      ...
      DO IT=1, LTCOM
       ...
       ELSE
         CTRACE(IT)=CMPLX(0.0,0.0)
       ENDIF
      END DO

      CALL FFT1D(CTRACE, LTCOM, 1)
      CALL HAMMING_WINDOW(CTRACE, NW1, NW2, NW3, NW4, LTCOM)

      DO IT=1, NW
         DATAW(IT) = CONJG(CTRACE(IT+NW1-1))
      END DO
...
*==========================================================================
      ...
      SUBROUTINE HAMMING_COE(IW, NW1, NW2, NW3, NW4, HMCOE)

      PARAMETER(PAI=3.14159165359)
      INTEGER  IW, NW1, NW2, NW3, NW4

       IF(IW.GE.NW1.AND.IW.LE.NW2) THEN
           HMCOE=0.54+0.46*COS(PAI*(IW-NW1)/(NW2-NW1)-PAI)
       ELSE IF(IW.GE.NW3.AND.IW.LE.NW4) THEN
           HMCOE=0.54+0.46*COS(PAI*(NW3-IW)/(NW4-NW3))
       ELSE IF(IW.GT.NW2.AND.IW.LT.NW3) THEN
           HMCOE=1.
       ELSE IF(IW.GT.NW4.OR.IW.LT.NW1) THEN
           HMCOE=0.
       END IF
